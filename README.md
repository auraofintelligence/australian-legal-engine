...
**Built and tested:** everything above, verified by 29 tests including a golden set that asserts real citations against the real 2012 acts (`python tests/test_engine.py`).

**Not built:** semantic search (the design keeps a slot for Australian legal embeddings, but the core stays dependency-light on purpose), a knowledge graph of amendments and cross-references, and any automated acquisition of law. The engine reads documents you already hold; it does no fetching of its own. That boundary is deliberate: the registers set terms on automated access, and asking is the path.
